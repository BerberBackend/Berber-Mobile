...
async function donemleriKontrolEt() {
    ...
    // Deneme süresi biten ve ödeme yapmayanları da durdur
    await pool.query(
        `UPDATE berber
         SET abonelik_durum = 'durduruldu'
         WHERE abonelik_durum = 'deneme' AND donem_bitis <= NOW()`
    );
}
...
function kullanimaAcikMi(abonelikDurum) {
    return abonelikDurum === 'deneme' || abonelikDurum === 'aktif';
}
